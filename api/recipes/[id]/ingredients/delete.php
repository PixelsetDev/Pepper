...
$data = new Request()->jsonValidated(['id']);

$recipe = $db->fetchOne("SELECT `author` FROM recipes WHERE `id` = ?", [$uriParts[3]]);
if (!$recipe || $recipe['author'] !== $decoded->sub) { echo new PepperResponse()->api(ResponseCode::Forbidden()); exit; }

$db->run("DELETE FROM recipes_ingredients WHERE `id` = ? AND `recipe_id` = ?", [$data->id, $uriParts[3]]);
