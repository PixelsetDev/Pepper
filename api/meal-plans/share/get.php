...
$query = $sql->query("SELECT `owner_uuid`, `shared_uuid` FROM `meal_plans_shares` WHERE (`owner_uuid` = '".$sql->escape($user->sub)."' OR `shared_uuid` = '".$sql->escape($user->sub)."') AND `pending` = 0");
if ($query->num_rows > 0) {
