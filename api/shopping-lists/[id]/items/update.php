...
$item = $db->fetchOne("SELECT `id`, `checked` FROM shopping_lists_items WHERE `id` = ? AND `list_uuid` = ?", [$uriParts[5], $uriParts[3]]);
...
$db->run("UPDATE shopping_lists_items SET `checked` = ? WHERE `id` = ?", [$item['checked'] ? 0 : 1, $uriParts[5]]);
